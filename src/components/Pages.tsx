...
export function ActivityPage({ activities, downloadingId, onDownload }: { activities: ActivityItem[]; downloadingId: string | null; onDownload: (activityId: string) => void }) {
  return <section className="activity-page" aria-labelledby="activity-title">
    <div className="activity-summary"><CheckCircle2 size={24} aria-hidden="true"/><div><strong>Recovery activity</strong><p>Latest verified backup and keep-alive events</p></div></div>
    <div className="panel activity-panel"><div className="panel-heading"><div><h2 id="activity-title">Recent activity</h2><p>Backup and keep-alive events</p></div><span className="badge">Last 7 days</span></div>
      {activities.length === 0 ? <div className="empty-state compact"><h3>No activity yet</h3><p>Runner events will appear here after the first job.</p></div> : activities.map(item => <div className="activity-row" key={item.id}><span className={`activity-icon ${item.type === 'keep_alive' ? 'amber' : ''}`} aria-hidden="true">{item.type === 'keep_alive' ? '⌁' : '✓'}</span><div className="activity-copy"><strong>{item.message}</strong><small>{item.projectId} · <time dateTime={item.occurredAt}>{formatDateTime(item.occurredAt)}</time></small></div><span className="activity-value">{item.bytes ? formatBytes(item.bytes) : formatDuration(item.durationMs)}</span>{item.type === 'backup' && item.status === 'success' && <button className="quiet action-button" type="button" disabled={Boolean(downloadingId)} onClick={() => onDownload(item.id)} aria-label={`Download backup for ${item.projectId} from ${formatDateTime(item.occurredAt)}`}><Download size={13}/>{downloadingId === item.id ? 'Preparing…' : 'Download'}</button>}</div>)}
    </div>
  </section>
}
...
export function SettingsPage() {
  return <section className="settings-stack" aria-labelledby="settings-title"><div className="panel narrow-panel"><div className="panel-heading"><div><h2 id="settings-title">Runtime</h2><p>Local-first recovery control plane.</p></div><span className="status"><b/>Connected</span></div><div className="settings-content"><dl><div><dt>Runner</dt><dd>Docker · PostgreSQL 17 tooling</dd></div><div><dt>Primary metadata</dt><dd>Local PostgreSQL · immediate transactions</dd></div><div><dt>Recovery mirror</dt><dd>Supabase PostgreSQL · daily atomic sync</dd></div><div><dt>Secret provider</dt><dd>AES-256-GCM local store · never mirrored</dd></div><div><dt>Backup destination</dt><dd>Cloudflare R2 · encrypted Restic repository</dd></div><div><dt>Retention</dt><dd>24 hourly · 7 daily · 4 weekly · 12 monthly</dd></div></dl><div className="warning-callout"><TriangleAlert size={17}/><p>Keep the Restic password and Vaultbase master key in an independent password manager. Neither can be recovered from R2.</p></div></div></div>
  </section>
}
