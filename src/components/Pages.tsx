...
function DatabaseCredentialForm({ onCancel, onSave }: { onCancel: () => void; onSave: (value: DatabaseCredentialsInput) => Promise<void> }) {
  ...
  return <CredentialForm saving={state.saving} error={state.error} onSubmit={state.submit} onCancel={onCancel}><div className="credential-form-grid"><label className="editor-field wide"><span>New Session Pooler URL</span><input type="password" autoComplete="new-password" minLength={20} value={value.sessionUrl} onChange={event => setValue(current => ({ ...current, sessionUrl: event.target.value }))} placeholder="Leave blank to keep the current Session credential"/><small>Default route over IPv4. Enter it only when adding or rotating this route.</small></label><label className="editor-field wide"><span>New Direct URL</span><input type="password" autoComplete="new-password" minLength={20} value={value.directUrl} onChange={event => setValue(current => ({ ...current, directUrl: event.target.value }))} placeholder="Leave blank to keep the current Direct fallback"/><small>Optional fallback requiring IPv6 or Supabase’s IPv4 add-on. Every entered route is tested before either secret is replaced.</small></label></div></CredentialForm>
}
