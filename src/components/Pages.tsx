    ...
    {projects.length === 0 ? <div className="empty-state compact"><KeyRound size={24}/><h3>No project credentials</h3><p>Add a project to create its encrypted credential references.</p></div> : <div className="credential-projects">{projects.map(project => {
      ...
      return <article className="credential-project" key={project.id}>
        ...
        {expanded && <div className="credential-rows" id={`credentials-${project.id}`}>
          <CredentialRow icon={<Database size={15}/>} title="Database connection" description="Used by pg_dump. Keep using the dedicated vaultbase_backup role." configured={project.secretConfigured} reference={project.secretPath} onEdit={() => setEditing({ projectId: project.id, kind: 'database' })} help={<><strong>Where to get it</strong><ol><li>Open this backup project in Supabase and select Connect.</li><li>Copy the Session Pooler connection string on port 5432.</li><li>Replace its username and password with your dedicated <code>vaultbase_backup</code> role, preserving the project reference suffix.</li><li>Percent-encode special password characters before saving.</li></ol><p><b>Real validation:</b> Vaultbase derives and matches the project reference, opens a TLS PostgreSQL connection, and executes <code>SELECT current_user</code>. It deliberately does not attempt a write; read-only safety comes from the role grants you created.</p></>}/>
