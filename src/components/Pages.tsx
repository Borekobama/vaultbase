    ...
    {projects.length === 0 ? <div className="empty-state compact"><KeyRound size={24}/><h3>No project credentials</h3><p>Add a project to create its encrypted credential references.</p></div> : <div className="credential-projects">{projects.map(project => {
      ...
      return <article className="credential-project" key={project.id}>
        ...
        {expanded && <div className="credential-rows" id={`credentials-${project.id}`}>
          <CredentialRow icon={<Database size={15}/>} title="Database connection" description="Used by pg_dump. Keep using the dedicated vaultbase_backup role." configured={project.secretConfigured} reference={project.secretPath} onEdit={() => setEditing({ projectId: project.id, kind: 'database' })} help={<><strong>Create or rotate the backup role</strong><p>Replace the password placeholder, then run this single statement in this backup project’s Supabase SQL Editor.</p><BackupRoleSqlTemplate/><strong>Build the connection string</strong><ol><li>Open this backup project in Supabase and select Connect.</li><li>Choose Direct when IPv6 is available, otherwise Session Pooler on port 5432.</li><li>Replace the default username and password with <code>vaultbase_backup</code> and its password. For Session Pooler, preserve the <code>.PROJECT_REF</code> suffix.</li><li>Percent-encode special password characters before saving.</li></ol><p><b>Real validation:</b> Vaultbase derives and matches the project reference, rejects the default <code>postgres</code> user, opens a TLS PostgreSQL connection, and executes <code>SELECT current_user</code>. It deliberately does not attempt a write; read-only safety comes from the role grants above.</p></>}/>
